# Script de deployment para STAGING (staging.shc.com.mx)
# Uso: .\scripts\deploy-staging.ps1

param(
    [string]$ServerUser = "root",
    [string]$ServerIP = "",
    [string]$ServerPath = "/var/www/ssc/staging"
)

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "DEPLOYMENT STAGING - staging.shc.com.mx" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# 2. Construir frontend para staging
Write-Host "[2/6] Construyendo frontend para staging..." -ForegroundColor Green
Set-Location "Client"
Copy-Item "env.staging.example" ".env" -Force
npm install
...
scp -r server/ecosystem.staging.config.cjs $ServerUser@${ServerIP}:${ServerPath}/backend/ecosystem.config.cjs
scp -r server/env.staging.example $ServerUser@${ServerIP}:${ServerPath}/backend/.env.example
...
pm2 stop ssc-backend-staging 2>/dev/null || true
...
Write-Host "========================================" -ForegroundColor Green
Write-Host "Staging disponible en: http://staging.shc.com.mx" -ForegroundColor Cyan
Write-Host ""
